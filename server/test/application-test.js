...
class FakeProblemRepository {
    ...
    async findById(problemId) {
        return this._problems.find(
            problem => problem.getId() === problemId
        );
    }
}
...
class FakeAttemptRepository {
    ...
    async findById(attemptId) {
        return this._attempts.find(
            attempt => attempt.getId() === attemptId
        );
    }
...
const elevatorProblem = new Problem({
    ...
    statement:
        "Design an elevator system that manages requests and movement.",
    difficulty: "hard",
    requirements: [
...
async function runTests() {
    ...
    try {
        ...
        throw new Error(
            "GetProblem should have rejected an unknown problem"
        );
    } catch (error) {
    ...
    const submissionResult = await submitDesign.execute({
        ...
        designEvidence: {
            ...
            assumptions: [
                "The parking lot has fixed capacity"
            ]
        }
    });
    ...
    console.log(
        "Evaluation status:",
        submissionResult.evaluation.status
    );
    ...
    try {
        ...
        throw new Error(
            "SubmitDesign should have rejected invalid predecessor"
        );
    } catch (error) {
    ...
    console.log(
        "Submission ID:",
        attemptDetails.submission.id
    );
    console.log(
        "Evaluation status:",
        attemptDetails.evaluation.status
    );
    ...
    const storedEvaluation =
        await evaluationRepository.findByAttemptId(
            storedAttempt.id
        );
    ...
    console.log(
        "Evaluation status after retry:",
        retriedEvaluation.status
    );
